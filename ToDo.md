...
**Mid term**
  ...
  test: Loading Whisper models cannot be cancelled and the call seems to fail when switching while a model is still loading
...
**Long term**
  ...
  reimplement stripping unnecessary dependencies for filesize eventually (this will probably not happen, since the apps are very lightweight now)