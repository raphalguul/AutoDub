...
def build(use_upx=True, standalone=False):
    ...
    if standalone:
        final_dir = DIST_DIR / f"wtdRenamer {version} standalone"
    else:
        final_dir = DIST_DIR / f"AutoDub Suite {version}"

    build_tmp = DIST_DIR / "_build_temp_wtdrenamer"
    ...
    src_exe = build_tmp / "wtdRenamer" / "wtdRenamer.exe"
    if not src_exe.exists():
        src_exe = build_tmp / "wtdRenamer.exe"
    if standalone:
        if final_dir.exists():
            shutil.rmtree(final_dir)
        final_dir.mkdir(parents=True)
        shutil.move(str(src_exe), str(final_dir / "wtdRenamer.exe"))
    else:
        if not final_dir.exists():
            final_dir.mkdir(parents=True)
        dst = final_dir / "wtdRenamer.exe"
        if dst.exists():
            dst.unlink()
...
def clean_build():
    ...
    tmp = DIST_DIR / "_build_temp_wtdrenamer"
    if tmp.exists():
        shutil.rmtree(tmp)


def print_summary(elapsed, output_dir: Path, version: str):
    exe = output_dir / "wtdRenamer.exe"
    if not exe.exists():
        return
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Build wtdRenamer")
